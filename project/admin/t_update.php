...
header('Refresh: 1; URL=teacher_info.php');
exit;
} 
...
if($_POST['name'] != null){
$value = $_POST['name'];
$sql = "UPDATE teacher_table SET teacher_name = ? WHERE teacher_id = ?";
$stmt = $db->prepare($sql);
$stmt->bind_param('si', $value, $t_id);
$stmt->execute();
}

if($_POST['gender'] != null){
$value = $_POST['gender'];
$sql = "UPDATE teacher_table SET gender = ? WHERE teacher_id = ?";
$stmt = $db->prepare($sql);
$stmt->bind_param('si', $value, $t_id);
$stmt->execute();
}
...
if($_POST['date'] != null){
$value = $_POST['date'];
$sql = "UPDATE teacher_table SET date_of_birth = ? WHERE teacher_id = ?";
$stmt = $db->prepare($sql);
$stmt->bind_param('si', $value, $t_id);
$stmt->execute();
}
...
$value = $_POST['address'];
$sql = "UPDATE teacher_table SET address = ? WHERE teacher_id = ?";
$stmt = $db->prepare($sql);
$stmt->bind_param('si', $value, $t_id);
$stmt->execute();
}
...
<link rel="stylesheet" type="text/css" href="../css/menu-admin.css">
    ...
    function clicked() {
       if (confirm('Are you sure you want to update teacher info?')) {
...
<p><a href="teacher_info.php">Go back</a></p>
...
<?php
$sql = "SELECT teacher_id, teacher_name, gender, date_of_birth, address FROM teacher_table WHERE teacher_id = ?";
$stmt = $db->prepare($sql);
$stmt->bind_param('i', $t_id);
$stmt->execute();
$result = $stmt->get_result();
$row = $result->fetch_assoc(); // Important line !!! Check summary get row on array ..
    ...
    foreach($row as $value) {
        ...
        <td>
          <input type="text" name="name" placeholder="UPDATE HERE"/>
        </td>
        <td>
        <select name="gender">
